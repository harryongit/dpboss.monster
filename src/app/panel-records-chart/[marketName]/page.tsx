...
export async function generateMetadata({ params }: { params: Promise<{ marketName: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${decodedMarketName} Panel Chart Record | DPBOSS Satta Matka`,
    description: `Check the latest and historical ${decodedMarketName} Panel chart records. DP BOSS provides accurate Satta Matka results for ${decodedMarketName}.`,
    keywords: `${decodedMarketName} panel chart, ${decodedMarketName} record, satta matka, dp boss, dpboss, matka result`,
    alternates: {
      canonical: `https://dpboss.monster/panel-records-chart/${resolvedParams.marketName}`,
    },
  };
}
